...
public class ModRecipeGenerator extends RecipeProvider implements IConditionBuilder {
    ...
    private static void simpleQuadPacker(Consumer<FinishedRecipe> pFinishedRecipeConsumer, ItemLike pUnpacked, ItemLike pPacked) {
        ...
        ShapedRecipeBuilder.shaped(RecipeCategory.MISC, pPacked, 1).define('#', pUnpacked).pattern("##").pattern("##").unlockedBy(getHasName(pPacked), has(pUnpacked)).save(pFinishedRecipeConsumer, new ResourceLocation(MigUtil.MOD_ID, getItemName(pPacked)));
        // unpacking (item from 2x2 block recipe)
        ShapelessRecipeBuilder.shapeless(RecipeCategory.MISC, pUnpacked, 4).requires(pPacked).group(null).unlockedBy(getHasName(pPacked), has(pPacked)).save(pFinishedRecipeConsumer, new ResourceLocation(MigUtil.MOD_ID, getConversionRecipeName(pUnpacked, pPacked)));
    }
    ...
    protected static void oreCooking(Consumer<FinishedRecipe> pFinishedRecipeConsumer, RecipeSerializer<? extends AbstractCookingRecipe> pCookingSerializer, List<ItemLike> pIngredients, RecipeCategory pCategory, ItemLike pResult, float pExperience, int pCookingTime, String pGroup, String pRecipeName) {
        for(ItemLike itemlike : pIngredients) {
            SimpleCookingRecipeBuilder.generic(Ingredient.of(itemlike), pCategory, pResult, pExperience, pCookingTime, pCookingSerializer)
                    ...
                    .save(pFinishedRecipeConsumer, MigUtil.MOD_ID + ":" + getItemName(pResult) + pRecipeName + "_" + getItemName(itemlike));
        }
    }
}
